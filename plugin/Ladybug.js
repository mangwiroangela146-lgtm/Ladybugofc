...
0        const pushname = m.pushName || "No Name"
        const botNumber = await XeonBotInc.decodeJid(XeonBotInc.user.id)
        const isOwner = [botNumber, ...settings.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        const text = args.join(" ")
        const quoted = m.quoted ? m.quoted : m
...
case 'help': {
    try {
        ...
        menuText += `📊 *Total Commands:* 35+\n`
        menuText += `🐞 *Powered by Ladybug v5.0.0*\n`
...
case 'searchanime': {
    if (!text) {
        await XeonBotInc.sendMessage(m.chat, { text: `❌ Please provide anime name!\n\nExample: ${prefix}animesearch Naruto` }, { quoted: m })
        break
    }
    ...
    try {
        ...
        if (data.data.length === 0) {
            await XeonBotInc.sendMessage(m.chat, { text: '❌ No anime found with that name!' }, { quoted: m })
            break
        }
        
        const anime = data.data[0]
        const animeInfo = `🎌 *ANIME SEARCH RESULT*\n\n` +
        `📺 *Title:* ${anime.title}\n` +
        `⭐ *Score:* ${anime.score || 'N/A'}\n` +
        `📅 *Year:* ${anime.year || 'N/A'}\n` +
        `🎭 *Type:* ${anime.type}\n` +
        `📊 *Status:* ${anime.status}\n` +
        `🏷️ *Genres:* ${anime.genres.map(g => g.name).join(', ')}\n` +
        ...
        await XeonBotInc.sendMessage(m.chat, { text: '❌ Failed to search anime' }, { quoted: m })
    }
    break
}
...
case 'manga': {
    try {
        ...
        const mangaInfo = `📚 *MANGA INFO*\n\n` +
        `📖 *Title:* ${manga.title}\n` +
        `⭐ *Score:* ${manga.score || 'N/A'}\n` +
        `📅 *Year:* ${manga.published?.from ? new Date(manga.published.from).getFullYear() : 'N/A'}\n` +
        `📊 *Status:* ${manga.status}\n` +
        `📝 *Synopsis:* ${manga.synopsis ? manga.synopsis.substring(0, 200) + '...' : 'No synopsis available'}`
        ...
        await XeonBotInc.sendMessage(m.chat, { text: '❌ Failed to fetch manga data' }, { quoted: m })
    }
    break
}
...
case 'nsfw': {
    ...
    if (!isAdmins && !isCreator) {
        await XeonBotInc.sendMessage(m.chat, { text: '❌ This command is only for admins!' }, { quoted: m })
        break
    }
    
    // Check if it's a group and NSFW is enabled
    if (m.isGroup && !db.data.chats[m.chat].nsfw) {
        await XeonBotInc.sendMessage(m.chat, { text: '❌ NSFW is disabled in this group!' }, { quoted: m })
        break
    }
...
// Additional NSFW toggle command for group admins
case 'nsfwtoggle':
case 'togglensfw': {
    if (!m.isGroup) {
        await XeonBotInc.sendMessage(m.chat, { text: '❌ This command is only for groups!' }, { quoted: m })
        break
    }
    
    if (!isAdmins && !isCreator) {
        await XeonBotInc.sendMessage(m.chat, { text: '❌ Only admins can toggle NSFW!' }, { quoted: m })
        break
    }
    
    const currentStatus = db.data.chats[m.chat].nsfw || false
    db.data.chats[m.chat].nsfw = !currentStatus
    
    const statusText = db.data.chats[m.chat].nsfw ? 'enabled' : 'disabled'
    await XeonBotInc.sendMessage(m.chat, { 
        text: `🔞 NSFW has been *${statusText}* in this group` 
    }, { quoted: m })
    break
}
...
case 'anime': {
    try {
        ...
        `🎭 *Type:* ${anime.type}\n` +
        `📊 *Status:* ${anime.status}\n` +
        `📝 *Synopsis:* ${anime.synopsis ? anime.synopsis.substring(0, 200) + '...' : 'No synopsis available'}`
        ...
        await XeonBotInc.sendMessage(m.chat, { text: '❌ Failed to fetch anime data' }, { quoted: m })
    }
    break
}
...
case 'neko': {
    try {
        ...
        await XeonBotInc.sendMessage(m.chat, {
            image: { url: data.url },
            caption: '🐱 *Random Neko*\n\nNya~ Here\'s a cute neko! 🐾'
        }, { quoted: m })
    } catch (error) {
        await XeonBotInc.sendMessage(m.chat, { text: '❌ Failed to fetch neko image' }, { quoted: m })
    }
    break
}
