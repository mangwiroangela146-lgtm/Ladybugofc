...
module.exports = async (XeonBotInc, m, chatUpdate, store) => {
    try {
        // Auto features - Execute before other processing
        
        // 1. Auto Always Online
        if (XeonBotInc.user && XeonBotInc.user.id) {
            try {
                await XeonBotInc.sendPresenceUpdate('available', m.chat)
            } catch (e) {
                console.log('Auto online error:', e)
            }
        }
        ...
        if (m.key && m.key.remoteJid === 'status@broadcast') {
            try {
                ...
                // Auto like/react to status (random reactions)
                const reactions = ['❤️', '😍', '🔥', '👍', '😊', '🎉', '💯', '⚡']
                ...
                console.log(chalk.green(`[AUTO] Viewed and liked status from: ${m.pushName || 'Unknown'}`))
            } catch (e) {
                console.log('Auto status view/like error:', e)
            }
        }
        ...
        // Add reply function with auto react
        const reply = async (text) => {
            try {
                // Auto react to user message first
                const userReactions = ['👍', '✅', '🤖', '⚡', '🐞']
                const randomUserReaction = userReactions[Math.floor(Math.random() * userReactions.length)]
                
                await XeonBotInc.sendMessage(m.chat, {
                    react: {
                        text: randomUserReaction,
                        key: m.key
                    }
                })
        ...
        // 4. Auto Recording for voice commands
        const voiceCommands = ['voiceclone', 'clonevoice', 'voicechange', 'changevoice', 'tts', 'say']
        if (voiceCommands.includes(command)) {
            try {
                await XeonBotInc.sendPresenceUpdate('recording', m.chat)
            } catch (e) {
                console.log('Auto recording error:', e)
            }
        }
            ...
            case 'help': {
                try {
                    ...
                    function generateLadybugMenu(categories, pushname, botname, prefix) {
                        ...
                        menuText += `🤖 *AUTO FEATURES ACTIVE:*\n`;
                        menuText += `• ⚡ Always Online Status\n`;
                        menuText += `• 👀 Auto Status View & Like\n`;
                        menuText += `• ⌨️ Auto Typing Indicator\n`;
                        menuText += `• 🎤 Auto Recording for Voice Commands\n`;
                        menuText += `• 👍 Auto React to Messages\n\n`;
...
case 'spotifydl': {
    ...
    break
       ...
       default:
        ...
        // Reset presence after command execution
        setTimeout(async () => {
            try {
                await XeonBotInc.sendPresenceUpdate('available', m.chat)
            } catch (e) {
                console.log('Reset presence error:', e)
            }
        }, 2000)
